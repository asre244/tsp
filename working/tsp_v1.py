...
def neighbors(distance_matrix, coordinate_list, points):
    # trim = len(points)  # todo: change this to dynamic
    decision_list = []
    last_choice = []
    for i in points:
...
def improvement(chosen_list_for_revision, total_distance):
    # finding the longest edge
    max_dist = max(i[1] for i in chosen_list_for_revision)
    # getting the connecting points of the longest edge
    max_distance_conn = [item for item in chosen_list_for_revision if item[1] == max_dist]
